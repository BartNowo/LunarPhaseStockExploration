...
# This file contains starts off by reading in moon data and Spy stock from csv and loads data into a DataFrme
...
spy_data_df['Date'] = pd.to_datetime(spy_data_df['timestamp']).dt.date

# merges the dataframes on the data column the inner means it will include only dates
# present in both dataframes
merged_spy_df = pd.merge(
    moon_phases_df, spy_data_df, on='Date', how='inner')

# creates a new dataframe that filters data for one yeat from jan 1 2020 t0 dec 31 2020
four_year_spy = merged_spy_df[(merged_spy_df['Date'] >= pd.to_datetime('2020-01-01').date()) &
                              (merged_spy_df['Date'] <= pd.to_datetime('2024-01-01').date())]

full_moon_dates = four_year_spy[four_year_spy['Moon Phase']
                                == 'Full Moon']['Date']
new_moon_dates = four_year_spy[four_year_spy['Moon Phase']
                               == 'New Moon']['Date']
...
first_new_moon_dates = find_first_days(four_year_spy, 'New Moon')
first_full_moon_dates = find_first_days(four_year_spy, "Full Moon")
...
for new_moon_date in first_new_moon_dates:
    ...
    if pd.notnull(following_full_moon):
        ...
        start_price = merged_spy_df.loc[merged_spy_df['Date']
                                        == new_moon_date, 'close']
        end_price = merged_spy_df.loc[merged_spy_df['Date']
